 ...
 * 1.插件的作用域：地图界面。
 *   只对事件有效，放置在 地图上层 。
 * 2.漂浮文字的图片层级与所有处于地图上层的相关插件的层级共享。
 *   （比如上层且图片层级大于100的地图背景，可以挡住漂浮文字，反之则在文字下面）
 * 3.你可以通过换事件页，来切换头顶的漂浮文字。
 *   你可以使用变量等特殊字符，但是变量的变动不会主动刷新漂浮文字。
 * 4.事件变形、位移时，漂浮文字会一直跟着事件。
 * 5.v1.3版本解决了需要时创建的bug，已固定为自动创建。
 *
 * -----------------------------------------------------------------------------
 * ----激活条件
 * 事件中，添加注释，在注释中填入以下指令：
 ...
 * 2.字符串中可以使用\V[n]变量，指令执行后变量的值会强制刷新到漂浮文字中。
 *   但是注释不会强制刷新。
 * 3.修改的文本离开当前地图后将失效。
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === '>事件漂浮文字') { 	//	>事件漂浮文字 : 本事件 : 修改文本 : 这是一串字符
		if(args.length == 6){
