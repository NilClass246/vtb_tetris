...
var AXY_Spine = {
	show: function () {
		...
		var objectname = filename.replace('-', '');
		...
		// load spine data
		if (!AXY.Spine.Param.spineObj[AXYSpineArgs['filename']]) {
			PIXI.loader
				.add(AXYSpineArgs['filename'], filename)
				.load(onAssetsLoaded);
		} else {
			SceneManager._scene.addChild(AXY.Spine.Param.spineObj[AXYSpineArgs['filename']]);
		}
		...
		function onAssetsLoaded(loader, res) {
			...
			//console.log(res['objectname']['spineData']);
			//console.log(res.objectname.spineData);
			// create a spine boy
			AXY.Spine.Param.spineObj[AXYSpineArgs['filename']] = new PIXI.spine.Spine(res[AXYSpineArgs['filename']]['spineData']);
			// set current skin
			//AXY.Spine.Param.spineObj[objectname].skeleton.setSkinByName('default');
			//AXY.Spine.Param.spineObj[objectname].skeleton.setSlotsToSetupPose();
			//AXY.Spine.Param.spineObj[objectname].skeleton.setToSetupPose();

			if (!AXY.Spine.Param.spinecontainer[id]) {
				AXY.Spine.Param.spinecontainer[id] = new PIXI.Container();
				SceneManager._scene.addChild(AXY.Spine.Param.spinecontainer[id]);
			}
			if (AXY.Spine.Param.spine[id] && AXY.Spine.Param.spine[id].filename == filename) {
				...
				AXY.Spine.Param.spinecontainer[id].removeChild(AXY.Spine.Param.spine[id]);
				AXY.Spine.Param.spine[id] = AXY.Spine.Param.spineObj[AXYSpineArgs['filename']];
				AXY.Spine.Param.spine[id].id = id;
				AXY.Spine.Param.spine[id].name = ('AXYSPINE');
				AXY.Spine.Param.spine[id].filename = filename;
				AXY.Spine.Param.spinecontainer[id].addChild(AXY.Spine.Param.spine[id]);
			...
			AXY.Spine.Param.spine[id].rotation = rotation;
			AXY.Spine.Param.spine[id].alpha = opacity;
			AXY.Spine.Param.spine[id].state.setAnimation(0, action, loop);
			AXY.Spine.Param.spine[id].action = action;
			//SceneManager._scene.addChild(AXY.Spine.Param.spinecontainer[id]);
	...
	action: function () {
		...
		if (AXY.Spine.Param.spinecontainer[id] && AXY.Spine.Param.spine[id]) {
			...
			switch (type) {
				case 'loop':
					AXY.Spine.Param.spine[id].state.setAnimation(0, action, true);
					AXY.Spine.Param.spine[id].action = action;
					break;
				case 'end':
					AXY.Spine.Param.spine[id].state.setAnimation(0, action, false);
					break;
				case 'temp':
				default:
					AXY.Spine.Param.spine[id].state.setAnimation(0, action, false);
					AXY.Spine.Param.spine[id].state.addAnimation(0, AXY.Spine.Param.spine[id].action, true, 0);
					break;
			}
			//AXY.Spine.Param.spine[id].state.setAnimationByName(0, action, loop);
			//if(!loop)
		...
		if (actorId) {
			...
			if (AXYSpineSVActorFileName) {
				...
				loader.onComplete = function () {
					...
					//AXY.Spine.Param.spine[actorId].id = id;
					//AXY.Spine.Param.spine[actorId].name = ('AXYSPINE');
					//AXY.Spine.Param.spine[actorId].filename = filename;
					//AXY.Spine.Param.spinecontainer[id].addChild(AXY.Spine.Param.spine[id]);
					//}
